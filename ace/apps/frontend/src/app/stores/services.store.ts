    ...
    withMethods((store, service = inject(ServicesService)) => ({
                ...
                switchMap((id: number) => {
                    return service.deleteService(id)
                        .pipe(
                            tapResponse({
                                next: () => {
                                    patchState(store, {services: store.services().filter(service => service.id != id)});
                                },
                                error: err => {
                                    console.error(err);
                                }
        ...
        validateService: rxMethod<number>(
            pipe(
                debounceTime(500),
                switchMap((id: number) => {
                    return service.validateService(id)
                        .pipe(
                            tapResponse({
                                next: (updatedService) => {
        ...
        updateService: rxMethod<RxUpdateService>(
            pipe(
                ...
                switchMap((rxUpdateService: RxUpdateService) => {
                    return service.updateService(rxUpdateService.id, rxUpdateService.updateServiceDTO)
                        .pipe(
                            tapResponse({
                                next: (updatedService) => {
                                    patchState(store, { services: store.services().map(service => 
